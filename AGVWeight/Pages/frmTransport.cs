...
namespace AGVWeight.Pages
{
    public partial class frmTransport : Form
    {
        ...
        {
            TransportDb productDb = new TransportDb();
            List<TransportModel> lists = productDb.getAllTransport();
            defineDatagrid(lists);
        }

        void defineDatagrid(List<TransportModel> lists)
        {
            dgv.Rows.Clear();
            if (lists == null)
                return;

            foreach (TransportModel list in lists)
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                TransportDb typeDb = new TransportDb();
                List<TransportModel> lists = typeDb.getTransportNameByLike(textBox1.Text);
                if (lists == null)
                {
